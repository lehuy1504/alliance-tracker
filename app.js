    ...
    let activeTab = 'view', sortCol = 'merit', numFmt = 'short', searchQuery = '';
    let cmpSrv = null, cmpD1 = null, cmpD2 = null;
    let cmpNumFmt = 'short', cmpSortKey = 'dm', cmpSearchQ = '', cmpTop10Key = 'dm';
    let _cmpDiffs = [];
    ...
    function _redrawCmpTable() {
      ...
      const sorted = [..._cmpDiffs].sort((a, b) => b[cmpSortKey] - a[cmpSortKey]);
    ...
    function buildCmpTable(filtered) {
      ...
      return `<table><thead><tr>
        ...
        ${allCols.map(c => `<th class="${cmpSortKey === c.k ? 'sorted' : ''}" onclick="setCmpSort('${c.k}')" style="min-width:110px">+/- ${c.l} ${cmpSortKey === c.k ? '↓' : '↕'}</th>`).join('')}
      </tr></thead>
    ...
    function renderView() {
      ...
      if (curServer && dates.length) {
        ...
        <div class="table-wrap"><table style="min-width:700px">
          <thead><tr><th style="width:42px">#</th><th style="width:60px">ID</th><th class="left" style="min-width:150px">Tên</th><th class="left" style="width:80px">Liên Minh</th>${cols.map(c => `<th class="${sortCol === c.k ? 'sorted' : ''}" onclick="setSortColFn('${c.k}')" style="width:105px">${c.l} ${sortCol === c.k ? '↓' : '↕'}</th>`).join('')}</tr></thead>
    ...
    window.setSortColFn = c => { sortCol = c; renderView(); };
    ...
    window.onSearch = q => {
      ...
      const sorted = [...rows].sort((a, b) => b[sortCol] - a[sortCol]); const filtered = filterRows(sorted); const isOn = !!q.trim();
    ...
    function buildTop10(diffs, key) {
      ...
      const top = [...diffs].sort((a, b) => b[key] - a[key]).slice(0, 10);
    ...
    window.setCmpSort = key => {
      cmpSortKey = key;
      const sorted = [..._cmpDiffs].sort((a, b) => b[key] - a[key]);
      const filtered = filterDiffs(sorted);
